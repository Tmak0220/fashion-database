...
function BrandEntry({ brand, centered = false }: { brand: Brand; centered?: boolean }) {
  ...
  return (
    <div className={align}>
      <p className={`type-label text-[11px] tabular-nums ${align}`}>
        {brand.start_year}
        {" — "}
...
function ColumnBrandEntry({ brand }: { brand: Brand }) {
  return (
    <div className="text-center">
      <BrandEntry brand={brand} centered />
      <CollapsibleDescription description={brand.description} centered />
    </div>
  )
}
...
function SplitRowView({ row }: { row: SplitRow }) {
  return (
    <div className="grid grid-cols-1 items-start gap-12 md:grid-cols-2 md:gap-24">
      <div
        style={{
          paddingTop: row.mens ? yearOffset(row.mens.start_year, row.anchorYear) : 0,
        }}
      >
        {row.mens ? <ColumnBrandEntry brand={row.mens} /> : null}
      </div>

      <div
        style={{
          paddingTop: row.womens ? yearOffset(row.womens.start_year, row.anchorYear) : 0,
        }}
      >
        {row.womens ? <ColumnBrandEntry brand={row.womens} /> : null}
      </div>
    </div>
  )
}
      ...
      <section className="mt-20">
        ...
        <div className="mt-10 space-y-10">
          {sortedBrands(brands).map((brand) => (
            <div key={brand.id}>
              <BrandEntry brand={brand} />
              <CollapsibleDescription description={brand.description} />
            </div>
          ))}
        </div>
      </section>
  ...
  return (
    <section className="mt-20">
      ...
      <div className="mt-10 grid grid-cols-1 gap-12 md:grid-cols-2 md:gap-24">
        <h3 className="text-center text-xs tracking-[0.08em] text-muted">メンズ</h3>
        <h3 className="text-center text-xs tracking-[0.08em] text-muted">ウィメンズ</h3>
      </div>
          ...
          if (row.kind === "both") {
            return (
              <div key={`both-${row.brand.id}`} className="text-center">
                ...
                <CollapsibleDescription description={row.brand.description} centered />
              </div>
            )
          }
