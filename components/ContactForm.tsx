    ...
    <div className="mt-12 max-w-4xl mx-auto">
      {status === "success" ? (
        <div className="p-6 border border-border bg-neutral-50 rounded-2xl text-sm leading-relaxed text-foreground">
          <p className="font-medium">{t("お問い合わせを受け付けました。")}</p>
          <p className="mt-2 text-muted text-xs">{t("通常2〜3営業日以内にご返信いたします。")}</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <p className="text-xs text-muted mb-2">{t("お名前を入力してください")}</p>
          ...
          <div>
            ...
            <p className="text-xs text-muted mb-2">{t("返信先のメールアドレスを入力してください")}</p>
          ...
          <div>
            ...
            <p className="text-xs text-muted mb-2">{t("ご質問やご要望、お気づきの点などをご記入ください")}</p>
            <textarea id="message" name="message" required rows={6} className="w-full border border-border rounded-xl px-4 py-3 bg-[#f5f4f2] text-foreground focus:outline-neutral-400 placeholder:text-neutral-400/70 resize-none leading-relaxed" placeholder={t("ご質問やご要望をご記入ください。")} />
          </div>
